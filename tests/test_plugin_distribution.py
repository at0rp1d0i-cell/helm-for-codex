...
BOOTSTRAP_SCRIPT = ROOT / "plugins" / "codex-ai-team" / "scripts" / "bootstrap_repo.py"


def test_plugin_manifest_and_marketplace_are_wired() -> None:
    manifest = json.loads((ROOT / "plugins" / "codex-ai-team" / ".codex-plugin" / "plugin.json").read_text())
    marketplace = json.loads((ROOT / ".agents" / "plugins" / "marketplace.json").read_text())

    assert manifest["name"] == "codex-ai-team"
    assert manifest["skills"] == "./skills/"
    assert manifest["interface"]["displayName"] == "Codex AI Team"
    assert marketplace["name"] == "codex-exploring-local"
    assert marketplace["plugins"][0]["name"] == "codex-ai-team"
    assert marketplace["plugins"][0]["source"]["path"] == "./plugins/codex-ai-team"
